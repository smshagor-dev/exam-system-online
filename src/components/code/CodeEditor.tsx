...
function HighlightedLine({ line }: { line: string }) {
  const parts = useMemo(() => {
    ...
    for (const match of line.matchAll(TOKEN_PATTERN)) {
      ...
      if (index > cursor) result.push({ text: line.slice(cursor, index), className: 'text-slate-100' })
      result.push({ text: match[0], className: tokenClass(match[0]) })
      cursor = index + match[0].length
    }
    if (cursor < line.length) result.push({ text: line.slice(cursor), className: 'text-slate-100' })
    return result.length ? result : [{ text: line || ' ', className: 'text-slate-100' }]
  }, [line])

  return <>{parts.map((part, index) => <span key={`${index}-${part.text}`} className={part.className}>{part.text}</span>)}</>
}

function LanguageBadge({ language }: { language: string }) {
  return <span className="rounded-md border border-slate-700 bg-slate-800 px-2 py-1 text-[11px] font-semibold uppercase tracking-wide text-slate-300">{language || 'text'}</span>
}

export function CodeBlock({ code, language, label = 'Code' }: { code: string; language: string; label?: string }) {
  const lines = (code || '').replace(/\r\n/g, '\n').split('\n')
  return (
    <div className="overflow-hidden rounded-xl border border-slate-700 bg-slate-950 shadow-sm">
      <div className="flex items-center justify-between border-b border-slate-800 bg-slate-900 px-4 py-2"><span className="text-xs font-semibold text-slate-300">{label}</span><LanguageBadge language={language} /></div>
      <div className="max-h-[32rem] overflow-auto">
        <div className="grid min-w-max grid-cols-[3.25rem_1fr] font-mono text-[13px] leading-6">
          <pre className="select-none border-r border-slate-800 bg-slate-900/70 px-3 py-4 text-right text-slate-600">{lines.map((_, index) => `${index + 1}\n`).join('')}</pre>
          <pre className="m-0 whitespace-pre px-4 py-4 text-slate-100">{lines.map((line, index) => <span key={index} className="block min-h-6"><HighlightedLine line={line} /></span>)}</pre>
        </div>
      </div>
    </div>
  )
  ...
  return (
    <div className="overflow-hidden rounded-xl border border-slate-700 bg-slate-950 shadow-sm">
      ...
      <div className="grid grid-cols-[3.25rem_1fr]" style={{ height: minHeight }}>
        <pre ref={gutterRef} aria-hidden className="m-0 overflow-hidden border-r border-slate-800 bg-slate-900/70 px-3 py-4 text-right font-mono text-[13px] leading-6 text-slate-600">{lines.map((_, index) => `${index + 1}\n`).join('')}</pre>
        <div className="relative min-w-0 overflow-hidden">
          <pre ref={highlightRef} aria-hidden className="pointer-events-none absolute inset-0 m-0 overflow-hidden whitespace-pre p-4 font-mono text-[13px] leading-6">{normalized.length === 0 ? <span className="text-slate-600">{placeholder}</span> : lines.map((line, index) => <span key={index} className="block min-h-6"><HighlightedLine line={line} /></span>)}</pre>
          <textarea
            ref={textareaRef}
            value={value}
