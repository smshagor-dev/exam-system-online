...
export default function AdminDashboardView({
  ...
  return (
    <div className="space-y-6 lg:space-y-8">
      <header className="rounded-[28px] border border-slate-200/80 bg-white px-5 py-5 shadow-[0_20px_45px_-30px_rgba(15,23,42,0.20)] sm:px-6">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex shrink-0 items-center gap-3 rounded-2xl border border-slate-200 bg-slate-50 px-3 py-2 sm:min-w-[180px]">
            ...
            <div className="min-w-0">
              ...
              <p className="truncate text-xs text-slate-500">{t('admin.dashboard.system_administrator', 'System Administrator')}</p>
            </div>
          </div>
        </div>
      </header>
      ...
      <section className="grid grid-cols-1 gap-6 xl:grid-cols-[1.6fr_0.9fr]">
        <div className="min-w-0 rounded-[28px] border border-slate-200/80 bg-white p-5 shadow-[0_18px_45px_-32px_rgba(15,23,42,0.22)] sm:p-6">
          <div className="flex flex-wrap items-center justify-between gap-3">
            ...
            <div className="rounded-full bg-blue-50 px-3 py-1 text-xs font-semibold text-blue-700">Jan - Dec</div>
          </div>

          <div className="mt-8 overflow-x-auto pb-2">
            <div className="flex h-[260px] min-w-[620px] items-end justify-between gap-3">
              {overviewData.map((item) => (
                <div key={item.month} className="flex min-w-9 flex-1 flex-col items-center justify-end gap-3">
          ...
          <div className="mt-6 space-y-4">
            {systemStatCards.map((item) => {
              ...
              return (
                <div
                  key={item.label}
                  className="flex items-center justify-between gap-3 rounded-2xl border border-slate-200 bg-slate-50 px-4 py-4 transition hover:border-blue-200 hover:bg-blue-50/60"
                >
                  ...
                  <span className="shrink-0 text-2xl font-semibold text-slate-950">{item.value}</span>
                </div>
              )
            })}
          </div>
      ...
      <section className="min-w-0 rounded-[28px] border border-slate-200/80 bg-white p-5 shadow-[0_18px_45px_-32px_rgba(15,23,42,0.22)] sm:p-6">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
        ...
        <div className="mt-6 overflow-x-auto">
          <table className="min-w-[680px] border-separate border-spacing-0 sm:min-w-full">
            <thead>
              <tr>
                {[
                  ...
                  <th key={heading} className="border-b border-slate-200 px-4 py-3 text-left text-[11px] font-semibold uppercase tracking-[0.18em] text-slate-500">
                    {heading}
                  </th>
                ))}
              </tr>
              ...
              {recentExams.length > 0 ? (
                recentExams.map((exam) => (
                  <tr key={`${exam.name}-${exam.date}`} className="transition hover:bg-slate-50/80">
                    <td className="border-b border-slate-100 px-4 py-4 text-sm font-medium text-slate-900">{exam.name}</td>
                    <td className="border-b border-slate-100 px-4 py-4 text-sm text-slate-600">{exam.subject}</td>
                    <td className="border-b border-slate-100 px-4 py-4 text-sm text-slate-600">{exam.date}</td>
                    <td className="border-b border-slate-100 px-4 py-4"><StatusBadge status={exam.status} /></td>
                  </tr>
                ))
              ) : (
                <tr>
      ...
      <footer className="flex flex-col gap-2 border-t border-slate-200 px-1 pt-4 text-sm text-slate-500 sm:flex-row sm:items-center sm:justify-between">
        <p>© {new Date().getFullYear()} {t('common.system_name', 'Exam Management System')}. {t('admin.dashboard.all_rights_short', 'All rights reserved.')}</p>
...
function StatusBadge({ status }: { status: RecentExam['status'] }) {
  ...
  return <span className={`inline-flex rounded-full px-3 py-1 text-xs font-semibold ring-1 ${styles}`}>{status}</span>
}
