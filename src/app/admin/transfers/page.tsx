...
export default async function TransfersPage() {
  ...
  const [transfers, activeEnrollments, departments, sessions, programs, years, semesters, programYears, programSemesters, departmentLanguages, groups] = await Promise.all([
    prisma.studentTransfer.findMany({
      where: scope.isSuperAdmin ? undefined : { fromDepartmentId: { in: scope.managedDepartmentIds } },
      include: {
  ...
  return (
    <StudentLifecycleActionWorkspace
      ...
      description="Move active students between groups, programs, departments, and language sections without losing academic history."
      actionEndpoint="/api/admin/transfers"
      ...
      fields={[
        ...
        { key: 'transferType', label: 'Transfer Type', type: 'select', required: true, options: ['GROUP', 'DEPARTMENT', 'PROGRAM'].map((value) => ({ value, label: value })) },
        ...
        { key: 'approvalNote', label: 'Approval Note', type: 'textarea' },
        { key: 'notes', label: 'Notes', type: 'textarea' },
      ]}
      recordsTitle="Transfer Log"
      recordsDescription="Closed source enrollments, activated target enrollments, and preserved transition history."
      columns={[
