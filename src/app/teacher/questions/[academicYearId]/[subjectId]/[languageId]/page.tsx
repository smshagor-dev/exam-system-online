...
export default async function SubjectQuestionBankPage({ params }: PageProps) {
  ...
  const language = profile.assignments[0].language
  ...
  return (
    <div className="space-y-6">
          ...
          <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
            <div>
              ...
              <p className="mt-2 text-sm text-gray-500">
                Language-specific question bank. Added questions below will stay under {language.name}.
              </p>
            </div>

            <div className="flex flex-wrap gap-2 text-xs text-gray-600">
              <span className="rounded-full bg-gray-100 px-3 py-1.5">Language: {language.name}</span>
      ...
      <QuestionBankManager
        questions={questions}
        createHref={`/teacher/questions/${academicYearId}/${subjectId}/${languageId}/new`}
      />
    </div>
  )
}
